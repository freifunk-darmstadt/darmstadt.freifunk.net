(function () {
    ...
    var width, height, largeHeader, canvas, canvasJq, ctx, points, target, mousepoint, animateHeader = true, point_count, max_distance = 0;
    ...
    function calculatePoints() {
        ...
        mousepoint.circle = new Circle(mousepoint, 0, 'rgba(246, 0, 115, 0.8)');
    ...
    function spawnPoint(px, py) {
        ...
        p.circle = new Circle(p, getRandomRange(min_point_radius, max_point_radius), 'rgba(246, 0, 115, 0.8)');
        p.circle.active = 0.7;
        points.push(p);
    }
    ...
    function resize() {
        if (width != $("#banner").outerWidth() || height != $("#banner").outerHeight()) {
            initValues();
            calculatePoints();
        }
    }
    ...
    function animate() {
        if (animateHeader) {
            ...
            if (canvas_opacity < 1) {
                ...
                canvasJq.css("opacity", canvas_opacity);
            }
        }
    ...
    function drawLines(p) {
        for (var i in p.closest) {
            ...
            if (quality_mode) {
                ...
                ctx.strokeStyle = 'rgba(246, 0, 115,' + p.closest[i].opacity + ')';
            }
            ctx.stroke();
        }
    }
